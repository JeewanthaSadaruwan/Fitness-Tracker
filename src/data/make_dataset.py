...
files = glob("../../data/raw/MetaMotion/*.csv")  # this specifies only csv files in the folder
len(files)  # 187 files
...
# python rstrip method removes any trailing(ending) characters
# where space is the default trailing character to remove 
...
for f in files:
    participant =f.split("-")[0].replace(data_path, "")  
    label = f.split("-")[1]
    category = f.split("-")[2].rstrip("123").strip("_MetaWear_2019")
